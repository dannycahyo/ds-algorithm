...
an array of positive integers representing durations of tasks that must be completed by the workers. 
...
Workers will complete their assigned tasks in parallel, 
and the time taken to complete all tasks will be equal 
to the time taken to complete the longest pair of tasks (see the sample output for an explanation).
...
export function taskAssignment(k: number, tasks: number[]) {
  const formattedTask = tasks.map((value, index) => ({
    value,
    index,
  }));
  formattedTask.sort((a, b) => a.value - b.value);

  const res = [];
  let start = 0,
    end = formattedTask.length - 1;

  while (start <= end) {
    res.push([formattedTask[start].index, formattedTask[end].index]);
    start++;
    end--;
  }
