...
one that contains the speeds of riders wearing red shirts and one that contains the speeds of riders wearing blue shirts. 
Each rider is represented by a single positive integer, which is the speed that they pedal a tandem bicycle at. 
Both lists have the same length, meaning that there are as many red-shirt riders as there are blue-shirt riders. Your goal is to pair every rider wearing a red shirt with a rider wearing a blue shirt to operate a tandem bicycle.
...
For example, if there are 4 riders (2 red-shirt riders and 2 blue-shirt riders) who have speeds of 1, 3, 4, 5, and if they're paired on tandem bicycles as follows: [1, 4], [5, 3], t
hen the total speed of these tandem bicycles is 4 + 5 = 9.
...
function tandemBicycle(
  ...
  if (fastest) {
    shortNums(redShirtSpeeds, "desc");
    shortNums(blueShirtSpeeds, "asc");
  } else {
    shortNums(redShirtSpeeds, "desc");
    shortNums(blueShirtSpeeds, "desc");
  }

  redShirtSpeeds.forEach((redSpeed, idx) => {
...
function shortNums(nums: number[], kind: "desc" | "asc") {
  return nums.sort((a, b) => {
    return kind === "asc" ? a - b : b - a;
  });
}
