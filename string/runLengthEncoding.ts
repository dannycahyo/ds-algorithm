...
export function runLengthEncoding(string: string) {
  ...
  if (string.length === 1) {
    // if the single character is a digit, return just that digit
  ...
  for (let i = 1; i < string.length; i++) {
    ...
    if (currentCharacter !== prevCharacter || currentLength === 9) {
      encodedStringCharacters.push(String(currentLength));
      encodedStringCharacters.push(prevCharacter);
